...
import { UserDetailsComponent } from './search/people/total/user-details/user-details.component';
...
export class FirstpageComponent  implements OnInit{
  ...
  fetchSavedSearchFilters(): void {
    this.getDataService.getSavedFilters().subscribe(
      (data: any[]) => {
        // Sort the filters by created_at property in descending order
        this.savedSearchFilters = data
          .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
          .slice(0, 3); // Get the three most recent filters
      },
      (error: any) => {
    ...
    if (userEmail) {
      this.getDataService.saveDataToUserAccount(userEmail, this.selectedRows).subscribe(
        (response) => {
          this.savedRecordsCount = response.saved_records_count;
          this.selectedRows = [];
          console.log(this.savedRecordsCount,'saverecordscount in firstpage');
